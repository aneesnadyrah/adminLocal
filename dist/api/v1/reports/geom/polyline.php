...
try {
    ...
    $leafletID = $POST['polylineId'];
    $features = $POST['geom'];

    $geom = 'SRID=4326;LINESTRING(' . implode(',', array_map(function ($coord) {
        return $coord[0] . ' ' . $coord[1];
    }, $features)) . ')';

    if ($POST['action'] == 'add') {
        $query = "INSERT INTO geom_site_visit (system_id, added_at, user_added, description, report_id, authority_id, geom, geom_id) VALUES
    (:systemId, :addedDate, :user, NULLIF(:notes, ''), :codedReport, :authId, (ST_GeomFromText(:geom)), :leafletID) RETURNING id";
        $stmt = $connPdo->prepare($query);
        $stmt->bindParam(':systemId', $systemId);
        ...
        $query = "UPDATE geom_site_visit SET description = NULLIF(:notes, ''), geom = (ST_GeomFromText(:geom)) WHERE report_id = :codedReport AND geom_id = :leafletID ";
        $stmt = $connPdo->prepare($query);
        $stmt->bindParam(':notes', $notes);
        $stmt->bindParam(':geom', $geom);
        $stmt->bindParam(':codedReport', $codedReport);
    ...
    if (isset($PUT['action'])) {
        ...
        // Update the record
        $query = "UPDATE geom_site_visit SET geom = ST_GeomFromText(:geom), added_at = :addedDate
...
// TODO: api for create
else if (isset($_POST['notes']) && isset($_POST['polylineId'])) {
    ...
    ('$systemId', '$addedDate', '$user', NULLIF('$notes', ''), '$codedReport', $authId, (ST_GeomFromText('$geom')), $leafletID) RETURNING id";
        ...
        $query = "UPDATE geom_site_visit SET description = NULLIF('$notes', ''), geom = (ST_GeomFromText('$geom'))
