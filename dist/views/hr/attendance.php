...
require_once "api/functions/attendance.php";
...
<body id="kt_body" style="background-image: url()" data-kt-aside-minimize="on" class="page-loading-enabled page-loading header-fixed header-tablet-and-mobile-fixed aside-fixed aside-secondary-enabled">
            ...
            <div class="wrapper d-flex flex-column flex-row-fluid" id="kt_wrapper">
                ...
                <div class="content d-flex flex-column flex-column-fluid" id="kt_content">
                    ...
                    <div class="container-xxl" id="kt_content_container">
                        <?php 
                            $letter->table('survey-attendance');
                            // include General::getTable("list-letter-in") 
    ...
    <script async src="assets/js/custom/letter-in/list.js"></script>
    <!--end::Custom Javascript-->
    <!--end::Javascript-->
    <script>
    $(document).ready(function() {
    $('#survey-attendance').DataTable({
...
document.getElementById('searchRujukan').addEventListener('keyup', function() {
    let input = this.value.toLowerCase();
    let rows = document.querySelectorAll('#survey-attendance tbody tr');

    rows.forEach(row => {
        let rujukanCell = row.querySelectorAll('td')[0]; // only check first column
        if (rujukanCell && rujukanCell.textContent.toLowerCase().includes(input)) {
            row.style.display = '';
        } else {
