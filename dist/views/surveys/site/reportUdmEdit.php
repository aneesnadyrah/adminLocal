...
$currentStatus = Survey::getcurrentStatus($_GET['sid']);
...
$startCoor = Survey::getStartCoordinates($systemId);
$endCoor = Survey::getEndCoordinates($systemId);
$surveyWorkData = Survey::getUpdateSurveyWorkDataUdm($systemId);
$totalDistance = Survey::totalDistanceUdm($systemId);

$surveyImageData = Survey::getSurveyImageDataReportUdm($systemId);
...
$teamId = Survey::getTeamSvyUdm($systemId);
...
foreach (Survey::surveyTeamModal() as $team) {
    ...
    // Check if the current team matches the selected team
    $isSelected = ($id == $teamId) ? 'selected' : '';

    $option .= '<option value="' . $id . '" data-profile-picture="' . General::getProfile($img) . '.jpg" ' . $isSelected . '> ' . $name . '</option>';
}
...
<body id="kt_body" style="background-image: url()" data-kt-aside-minimize="on" class="page-loading-enabled page-loading header-fixed header-tablet-and-mobile-fixed aside-fixed aside-secondary-enabled">
            ...
            <div class="wrapper d-flex flex-column flex-row-fluid" id="kt_wrapper">
                ...
                <div class="content d-flex flex-column flex-column-fluid" id="kt_content">
                    ...
                    <div class="container-xxl" id="kt_content_container">
                        <!--begin::Row-->
                        <?php 
                        if (in_array(48, array_column($currentStatus, 'status_id'))) {
                            include "components/forms/survey-udm-report-confirm.php";
                        } else {
                            // include "components/forms/survey-udm-report-review.php";
                            include "components/forms/update-survey-udm-report.php";
                        }
                        ?>
                        <!--end::Row-->
                    </div>
    ...
    <script src="assets/js/custom/survey/site/report-udm-edit.js"></script>
    ...
    $(document).ready(function() {
        ...
        var surveyData = <?php echo json_encode($surveyWorkData); ?>;
        
        // Initialize Select2 for survey work
        $('select[name="survey-work[]"]').select2();
        // Set the selected values
        $('select[name="survey-work[]"]').val(surveyData.survey_work).trigger('change');
        
        // Initialize Select2 for equipment
        $('select[name="equipment[]"]').select2();
        // Set the selected values
        $('select[name="equipment[]"]').val(surveyData.equipment).trigger('change');
    });
    </script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const defaultDate = dateInput.dataset.date; // Get date from data attribute
        ...
        const dateInputE = document.querySelector('#dateEnd');
        const defaultDateE = dateInputE.dataset.date; // Get date from data attribute
        
        flatpickr(dateInputE, {
            dateFormat: "Y-m-d",
    ...
    <!-- <script>
        document.addEventListener("DOMContentLoaded", function () {
            const surveyImageCount = document.getElementById("surveyImageCount");
            const addRepeater = document.getElementById("add-repeater");
            const addTambahBtn = document.getElementById("add-tambah");
            
            if(surveyImageCount.value ===  "1") {
                addRepeater.style.display = "none"; // Hide add-repeater
                addTambahBtn.style.display = "block"; // Show add-tambah
            } else {
                addRepeater.style.display = "block"; // Show add-repeater
                addTambahBtn.style.display = "none"; // Hide add-tambah
            }
        });
    </script> -->
