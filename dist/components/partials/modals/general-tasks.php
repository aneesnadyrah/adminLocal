...
foreach ($data as $item) {
    $size = isset($item->aside)? "modal-xl" : "";
    $visible = isset($item->input->upload)? 'd-none' : '';
    $title = isset($item->title)? $item->title : 'Tindakan Tugasan';
        ...
        echo '<div class="modal-dialog modal-dialog-centered '. $size .'">';
            echo '<div class="modal-content">';
                echo '<div class="modal-header">';
                    ...
                    echo '<div class="btn btn-icon btn-sm btn-active-light-secondary ms-auto" data-bs-dismiss="modal" aria-label="Close">';
                        ...
                        echo '<div class="d-flex flex-row">';
                            if (isset($item->aside)) {
                                ...
                                if($attachment->url !== NULL && $attachment->url !== "") {
                                    ...
                                    echo '<div class="min-w-600px h-800px me-5">';
                            ...
                            echo '<div class="w-100">';
                                ...
                                // Pinda Sebut Harga
                                if(isset($item->input->note)) {
                                    foreach ($item->input->note as $note) {
                                        echo '<div class="row mb-5 fv-row">';
                                            echo '<label class="card-title form-label">' . $note->label . '</label>';
                                            echo '<span class="text-gray-700">' . ($note->note ? $note->note : '- Tiada -') . '</span>';
                                        echo '</div>';
                                    }
                                }
                                ...
                                if(isset($item->input->checkbox)){
                                    ...
                                    foreach($item->input->checkbox as $index => $checkbox){
                                        ...
                                        if ($item->input->checkbox->$index->type === "checkbox" || $item->input->checkbox->$index->type === "radio") {
                                            ...
                                            echo '<input class="form-check-input" type="'. $checkbox->type .'" value="'. $checkbox->value .'" name="'.$checkbox->name .'" id="checkbox-'. $checkbox->name .'" '. ($checkbox->checked ?? '') .'/>';
                                            echo '<label class="form-check-label  text-gray-700" for="checkbox-'. $checkbox->name .'">'. $checkbox->label .'</label>';
                                            echo '</div>';
                                            echo '</div>';
                                        }
                                        ...
                                        foreach ($item->input->select as $index => $select) {
                                            if($item->input->select->$index->type === 'images') {
                                                ...
                                                foreach ($select->option as $option) {
                                                    ...
                                                    $images = !isset($option->url) ? '' : 'data-select2-images="'.$option->url . '"';
                                ...
                                if(isset($item->input->date)) {
                                    ...
                                    foreach ($item->input->date as $date) {
                                        ...
                                        $placeholder = ($date->type === 'time') ? 'Sila Pilih Masa' : 'Sila Pilih Tarikh';
                                        echo '<input type="date" class="form-control form-control-solid" name="' . $date->name . '" placeholder="'.$placeholder.'" data-control="flatpickr" data-flatpickr-type="'. $date->type .'" '. $required .'/>';
                                        echo '</div>';
                                    }
                                    echo '</div>'; // Close the row
                                }
                                ...
                                if(isset($item->input->text)) {
                                    ...
                                    foreach ($item->input->text as $text) {
                                        ...
                                        $placeholder = isset($text->placeholder) ? $text->placeholder : $text->label;
                                        echo '<div class="mb-5 col-md-' . $columns . ' fv-row">';
                                        echo '<label class="form-label '. $required .'">' . $text->label . '</label>';
                                        echo '<input type="text" class="form-control form-control-'.$style.'" '. $attribute .' name="' . $text->name . '" value="' . ($text->value ?? '') . '" placeholder="' . $placeholder . '" '. $required .' '. $readonly.' />';
                                        echo '</div>';
                                    }
                                    echo '</div>'; // Close the row
                                }
                                ...
                                if(isset($item->input->textarea)) {
                                    ...
                                    foreach ($item->input->textarea as $textarea) {
                                        ...
                                        echo '<textarea class="form-control form-control-solid" rows="5" name="' . $textarea->name . '"></textarea>';
                                        echo '</div>';
                                    }
                                    echo '</div>'; // Close the row
                                }
