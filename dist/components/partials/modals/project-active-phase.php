...
foreach ($data as $item) {
    ...
    echo '<div class="mb-5"><p class="fs-4">Tugasan untuk <strong>'.$item->action.'</strong> akan dicipta bagi permohonan ini.</p></div>';
    ...
    if (isset($item->input->text)) {
        ...
        foreach ($item->input->text as $text) {
            ...
            echo '<input type="text" class="form-control form-control-' . $style . '" ' . $attribute . ' name="' . $text->name . '" placeholder="' . $placeholder . '" ' . $required . ' ' . $readonly . ' />';
            echo '</div>';
        }
        echo '</div>'; // Close the row
    }
