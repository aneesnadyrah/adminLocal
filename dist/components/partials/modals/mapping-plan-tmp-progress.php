...
foreach ($data as $item) {
    ...
    $progress = intval(Survey::getProgressTMP($id->system_id));
    $surveyLength = $surveyApi->getSurveyLength($systemId);
    $progressTMP = intval(($progress / $surveyLength) * 100);
    //tmp_date = 0 
    echo '<!--begin::Modal-->  
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h3 class="modal-title">Kemaskini Progress Pelan Kawalan Trafik</h3>
                ...
                <form class="modal-body" novalidate="novalidate" data-form="form-' . $item->id . '">
                    ...
                    <div class="d-flex flex-column w-100 me-2 mb-7">
                        <div class="d-flex flex-stack mb-2">
                            ...
                            <span class="text-muted me-20 fs-7 fw-bold">' . $progressTMP . '%</span>
                        </div>
                        <div class="progress w-100" style="height: 10px;"><div class="progress-bar ' . ($progressTMP <= 25 ? 'bg-danger' : ($progressTMP <= 50 ? 'bg-warning' : ($progressTMP <= 75 ? 'bg-info' : 'bg-success'))) . ' progress-bar-striped progress-bar-animated" role="progressbar" style="width: ' . $progressTMP . '%" aria-valuenow="' . $progressTMP . '" aria-valuemin="0" aria-valuemax="100"></div>
                        </div>
                    </div>
                    <!--end::Progress-->
                    ...
                    <div class="input-group mb-7 fv-row">
                        ...
                        <input type="text" class="form-control" id="progress-daily-tmp"  name="progress-daily-tmp" aria-describedby="progress-daily-tmp"/>
                        <input type="text" name="progress-tmp" value="' . $progress . '" hidden />
