    ...
    foreach ($item->charts AS $index => $chart) {
        ...
        echo <<<TEMPLATE
                <div class="tab-pane fade $isActive" id="$chart->tab" role="tabpanel">
                    <div class="w-100 p-5" data-radial-chart="$chart->id" data-radial-labels='$chart->labels' data-radial-series="$chart->series" data-radial-style="$chart->style" data-radial-height="$chart->height"></div>
                </div>
    TEMPLATE;
    }
