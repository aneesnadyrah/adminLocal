 ...
 <div class="card card-flush">
     ...
     <div class="card-body pt-0">
            <?php
            $row = Wayleave::wyMKIL($_GET['sid'])[0];
            $total = Wayleave::countWy($_GET['sid'], 1);
            $total_status = Wayleave::countWy($_GET['sid'], 3);

            if($appsTitle == 'UCIDOS' || $appsTitle == 'KUDRAT') {
                if($total_status < $total) { ?>
                    <div class="d-flex justify-content-end">
                        <button type="button" href="#" class="btn btn-light-primary w-40px w-md-auto h-40px px-0 px-md-6 pb-4" data-bs-toggle="modal" data-bs-target="#action-<?php echo $row['WyStatus']. $row['WyID']?>" disabled>
                            <i class="fad fa-circle-plus fs-4"></i>
                            <span class="d-none d-md-inline">Jana MKIL</span>
                        </button>
                    </div>
            <?php
                } else { ?>
                    <div class="d-flex justify-content-end">
                        <button type="button" href="#" class="btn btn-light-primary w-40px w-md-auto h-40px px-0 px-md-6 pb-4" data-bs-toggle="modal" data-bs-target="#action-<?php echo $row['WyStatus']. $row['WyID']?>">
                            <i class="fad fa-circle-plus fs-4"></i>
                            <span class="d-none d-md-inline">Jana MKIL</span>
                        </button>
                    </div>
         ...
         <!--begin::Table-->
         <table class="table align-middle table-row-dashed fs-6 gy-5" id="wayleave-feedback">
             <!--begin::Table head-->
             <thead>
                 <!--begin::Table row-->
