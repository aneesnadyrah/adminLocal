var review = (function () {
  ...
  let htaccessUrlParams = urlWithOutParams
    .replace(apps + "/reports/site/review", "")
    .split("/");
  ...
  const reviewFormEl = document.getElementById("report-review");
  const printBtn = document.querySelector('[data-report-view="print"]');
  const wlCheckboxEl = reviewFormEl.querySelector('[name="proceed-wl"]');
  const amendCheckboxEl = reviewFormEl.querySelector('[name="proceed-amend"]');
  const notesEl = reviewFormEl.querySelector('[name="notes"]');
  const submitButton = document.getElementById("report-review-submit");
  autosize(notesEl);

  // Initialize amendStatus to false
  let amendStatus = false;
  wlCheckboxEl.checked = true;

  // Listen to print button
  printBtn.addEventListener("click", function () {
    alert("print button is clicked");
  });

  // Listen to checkbox events
  wlCheckboxEl.addEventListener("change", function () {
    if (this.checked) {
      // Checkbox is checked
      // Perform actions for checked state
      console.log("wlCheckboxEl is checked");
      if (amendStatus == true) {
        alert(
          "Terdapat butiran pindaan pada laporan. Adakah anda pasti untuk teruskan proses Izin Lalu?"
        );
      }
      ...
      console.log("wlCheckboxEl is unchecked");
      amendCheckboxEl.checked = true;
      amendCheckboxEl.dispatchEvent(new Event("change"));
      // Do something...
    }
  });

  amendCheckboxEl.addEventListener("change", function () {
    if (this.checked) {
      // Checkbox is checked
      // Perform actions for checked state
      console.log("amendCheckboxEl is checked");
      if (amendStatus == false) {
        alert("Tiada sebarang butiran pindaan pada laporan. Adakah anda pasti untuk teruskan Pindaan?");
      }
      wlCheckboxEl.checked = false;
      // Do something...
    } else {
      // Checkbox is unchecked
      // Perform actions for unchecked state
      console.log("amendCheckboxEl is unchecked");
      wlCheckboxEl.checked = true;
      wlCheckboxEl.dispatchEvent(new Event("change"));
      // Do something...
    }
  });

  // Iterate through the array and check amend_status
  for (const obj of reportData) {
    if (obj.amend_status == 1) {
      amendStatus = true;
      amendCheckboxEl.checked = true;
      // Trigger the "change" event on the checkbox element
      amendCheckboxEl.dispatchEvent(new Event("change"));
  ...
  // Hightlighter control
  function hightlightAmendControl() {
    const hlCheckBoxEl = document.getElementById("highlighter");

    // check for element with amend data
    const amendElements = document.querySelectorAll('[data-report-review="hightlight"]');

    if (amendElements.length == 0) {
      hlCheckBoxEl.checked = false;
    } else {
      hlCheckBoxEl.removeAttribute('disabled');
      amendElements.forEach((element) => {
        element.classList.add("bg-warning");
      });
    }

    hlCheckBoxEl.addEventListener("change", function () {
      if (this.checked) {
        // Checkbox is checked
        // Perform actions for checked state
        console.log("hlCheckboxEl is checked");
        amendElements.forEach((element) => {
          element.classList.add("bg-warning");
        });
        // scroll to view on highlight on
        amendElements[0].scrollIntoView({ behavior: 'smooth', block: 'center', inline: 'center' });
      } else {
        // Checkbox is unchecked
        // Perform actions for unchecked state
        console.log("hlCheckboxEl is unchecked");
        amendElements.forEach((element) => {
  ...
  function updateFormData() {
    formData = new FormData(reviewFormEl); // Update the global formData with the latest form data
    return formData;
  }
  ...
  reviewFormEl.addEventListener("submit", function (event) {
    ...
    // Perform any custom actions or validations here
    // For example, you can access form data using:
    formData = updateFormData();

    // Convert formData to JSON object
    const jsonFormData = formDataToJson(formData);
    jsonFormData.item = "report-review";
    jsonFormData.report = reportData;
    jsonFormData.reportCtrl = reportCtrl;
    
    // console.log(jsonFormData);

    api.post(`reports/${systemId}`,jsonFormData).then((response) => {
      toastr.success(response.message);

      setTimeout(function () {
        location.href = "management/general/tasks/new";
      }, 2500);
    }).catch((error) => {
      console.log(error);
    })
    ...
    // Here's an example of an alert to show that the submission is prevented
    // alert("Form submission prevented!");

    // Optionally, you can now submit the form programmatically
    // after performing your custom actions
    // event.target.submit();
  });
  ...
  function mapSetup() {
    ...
    mapData.forEach(function (mapReportData) {
      ...
      api
        .get(
          `reports/geometry/marker?action=init&sid=${systemId}&ref=${reportNo}&auth=${mapReportData.authority_id}`
        )
        .then((response) => {
      ...
      api
        .get(
          `reports/geometry/polygon?action=init&sid=${systemId}&ref=${reportNo}&auth=${mapReportData.authority_id}`
        )
        .then((response) => {
      ...
      api
        .get(
          `reports/geometry/polyline?action=init&sid=${systemId}&ref=${reportNo}&auth=${mapReportData.authority_id}`
        )
        .then((response) => {
