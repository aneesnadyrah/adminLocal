...
var generalTable = (function () {
  var dataTable, flatpickr, filterSearch, filterStatus, minDate, maxDate, clearButton;
  ...
  var initTable = () => {
    return new Promise((resolve) => {
      dataTable = new DataTable(table, {
        data: Data, // Initialize data here
        columns: [
            { data: 'reference_no', title: 'No Rujukan' },
            { 
                data: 'utility', 
                title: 'Penyedia Utiliti',
                render: function (data, type, row) {
                    // return data;
                           return '<div class="" data-bs-toggle="tooltip" data-bs-placement="top" title="'+data.name+'">'+
            ...
            { 
                ...
                render: function (data, type, row) {
                    // return data;
                           return '<div class="" data-bs-toggle="tooltip" data-bs-placement="top" title="'+data.name+'">'+
                                '<div class="symbol symbol-50px">'+
                                    '<img src="assets/media/authorities/'+data.logo+'.png" alt="'+data.name+'"  />'+
                                '</div>'+
                            '</div>'+
                            '<span class="d-none">'+data.name+'</span>'
                }
            ...
            { 
                ...
                title: 'Daerah',
                render: function (data, type, row) {
                    return '<span class="badge badge-secondary me-2">'+data+'</span>';
                }
            },
            { data: 'tarikh', title: 'Tarikh' },
            { data: 'masa', title: 'Masa' },
            { data: 'location', title: 'Lokasi' },
            { data: 'description', title: 'Nota' }
        ],
        info: false,
        language: {
