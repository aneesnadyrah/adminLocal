...
var KTApp = function () {
    ...
    var createSelect2 = function () {
        ...
        elements.map(function (element) {
            ...
            if(element.getAttribute('data-show-images') == 'true') {
                const optionFormat = (item) => {
                    ...
                    if(item.element.getAttribute('data-select2-images') !== null){
                        template += '<img src="' + item.element.getAttribute('data-select2-images') + '" class="rounded-circle h-40px me-3" alt="' + item.text + '"/>';
                    } else {
                        // For Avatar badge
                        var stateNum = Math.floor(Math.random() * 6);
                        var states = ['success', 'danger', 'warning', 'info', 'primary', 'dark'];
                        var $state = states[stateNum],
                            $name = item.text;
                        var $initials = $name.match(/\b\w/g) || [];
                        $initials = (($initials.shift() || '') + ($initials.pop() || '')).toUpperCase();

                        template += `<div class="symbol symbol-circle symbol-40px"><div class="symbol-label fs-5 me-3 fw-semibold text-${ $state }">${ $initials }</div></div>`;
                        
                    }
                    template += '<div class="d-flex flex-column">'
                    template += '<span class="fs-4 fw-bold lh-1">' + item.text + '</span>';
                    if(item.element.getAttribute('data-select2-desc') !== null){
                        template += '<span class="text-muted fs-5">' + item.element.getAttribute('data-select2-desc') + '</span>';
                    };
                    template += '</div>';
                    template += '</div>';
    ...
    var createFlatpickr = function () {
        ...
        var elements = [].slice.call(document.querySelectorAll('[data-control="flatpickr"], [data-kt-flatpickr="true"]'));

        elements.map(function (element) {
            if (element.getAttribute("data-kt-initialized") === "1") {
                return;
            }
            var type = element.getAttribute("data-flatpickr-type");
