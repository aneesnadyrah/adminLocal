var prints = (function () {
  ...
  if (urlWithOutParams.includes("prints")) {
    ...
    let htaccessUrlParams = urlWithOutParams
    .replace(apps + "/projects/view/LLT", "")
    .split("/");
    // urlParams[0] is ignore as it handle the first / in the trimmed url
    systemId = htaccessUrlParams[1];
  }
  ...
  function mapSetup() {
    ...
    //LAYER REQUEST
    var chartingLine = L.Geoserver.wfs("https://geos.asiadebut.tech/wfs", {
      pmIgnore: true,
    ...
    api
      .get(
        `reports/geometry/marker?action=init&sid=${systemId}&ref=${reportNo}&auth=${authorityId}`
      )
      .then((response) => {
        ...
        response.markers.forEach((element) => {
          ...
          marker.on("pm:remove", function () {
            ...
            api
              .delete(
                `reports/geometry/marker?id=${markerRowId}&markerId=${marker._leaflet_id}&sid=${systemId}&ref=${reportNo}&auth=${authorityId}`
              )
              .then((response) => {
                toastr.options = optionToast;
                // Show a success message to the user
    ...
    api
      .get(
        `reports/geometry/polygon?action=init&sid=${systemId}&ref=${reportNo}&auth=${authorityId}`
      )
      .then((response) => {
        ...
        response.polygon.forEach((element) => {
          ...
          polygon.on("pm:remove", function () {
            ...
            api
              .delete(
                `reports/geometry/polygon?sid=${systemId}&ref=${reportNo}&auth=${authorityId}&id=${element.id}&polygonId=${polygon._leaflet_id}`
              )
              .then((response) => {
                toastr.options = optionToast;
                // Show a success message to the user
    ...
    api
      .get(
        `reports/geometry/polyline?action=init&sid=${systemId}&ref=${reportNo}&auth=${authorityId}`
      )
      .then((response) => {
        ...
        response.polyline.forEach((element) => {
          ...
          polyline.on("pm:remove", function () {
            ...
            api
              .delete(
                `reports/geometry/polyline?sid=${systemId}&ref=${reportNo}&auth=${authorityId}&id=${element.id}&polylineId=${polyline._leaflet_id}`
              )
              .then((response) => {
                toastr.options = optionToast;
                // Show a success message to the user
