var createSummary = (function () {
  ...
  //   var authorityID = urlParams.get('aid');
  ...
  api
    ...
    .then((response) => {
      ...
      authorityId = response.authority_id.map((item) => ({
        ...
        road: item["road_name"]
          ...
          : [],
        actualLength: item.involved_appl_length
      }));
  ...
  function modalContent(authorityIdd) {
    // Stepper lement
    let stepperModal = "#kt_stepper_project_summary";
    // Stepper lement
    ...
    authTest.forEach((element) => {
      ...
      let stepperContentEl = `<div class="flex-column " data-kt-stepper-element="content">
                          ...
                          <div class="d-flex flex-row align-items-center mb-5 fv-row fv-plugins-icon-container">
                              <span class="col-4">Jangkaan Tempoh Kerja di Tapak</span>
                              <span class="col-1">:</span>
                              <div class="col-5">
                                  <input id="work-on-site-${element.id}" name="work-on-site-${element.id}" type="text"
                                      class="form-control" placeholder="Sila Isi Jangkaan Tempoh Kerja"/>
                              </div>
                                                          ...
                                                          <div data-repeater-item>
                                                              <div class="form-group row">
                                                                  <div class="col-md-7 mb-5 fv-row">
                                                                      ...
                                                                      <select name="road-method-hdd-${element.id}"
  ...
  function roadRepeaterInit(
    ...
    // initialize repeater
    authorityIdd.forEach((element) => {
      $(`#method-involved-${element.id}`).repeater({
        initEmpty: false,
  ...
  function totalLengthRepeaterInit(authoirtyId) {
    authoirtyId.forEach((element) => {
      ...
      function calculateLength() {
        ...
        // if (totalSum === 1000) {
        //   console.log("masuk dalam event ni");
        // }
        return totalSum;
      }
  ...
  let formId = "#form-project-summary";
  let form = document.querySelector(formId);
  // let formData = JSON.stringify(form);

  const submitButton = document.querySelector("#submit-project-summary");
  ...
  submitButton.addEventListener("click", function (e) {
    ...
    submitButton.disabled = true;
        ...
        authorityId.forEach((element) => {
          ...
          // console.log(element.actualLength);

          if (element.totalLength == element.actualLength) {
            console.log('Sama');
            element.proceed = true;
          } else {
            console.log('Tak Sama');
            element.proceed = false;
          }
        });
        ...
        authorityId.forEach((element) => {
          if (!element.proceed) {
            notice.push(`Jarak bagi Pihak Berkuasa <strong>${element.name}</strong> berbeza sebanyak <span class="badge badge-danger">${element.totalLength - element.actualLength} meter </span>`);
          }
        });

        const noticeString = notice.join('<br><br>');
      ...
      if (result.isConfirmed) {
        ...
        api
          ...
          .then((response) => {
            ...
            setTimeout(function () {
              location.href = `/projects/wayleave/summary/` + response.systemId;
            }, 2500);
          })
          .catch((error) => {
            // Hide loading indication
